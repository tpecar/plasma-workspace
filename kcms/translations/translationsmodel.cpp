...
QVariant TranslationsModel::data(const QModelIndex &index, int role) const
{
    ...
    case LanguageCode:
        return code;
    case IsMissing:
        return lang->state == Language::State::Missing;
    case IsIncomplete:
...
void TranslationsModel::setSelectedLanguages(const QStringList &languages)
{
    ...
    for (const QString &code : languages) {
        if (const int index = m_languageCodes.indexOf(code); index >= 0) {
            ...
            Q_EMIT dataChanged(modelIndex, modelIndex, {IsSelected});
        } else {
#warning TODO if we stopped modeling missing languages we could const the language list and I am not sure the modeling adds much value anyway
            beginInsertRows(QModelIndex(), m_languages.size(), m_languages.size());
            m_languageCodes << code;
            m_languages << new Language{code, Language::State::Missing, this};
            endInsertRows();
...
QStringList TranslationsModel::missingLanguages() const
{
    ...
    for (const auto &lang : m_languages) {
        if (lang->state == Language::State::Missing) {
            missing << lang->code;
        }
    }
    return missing;
