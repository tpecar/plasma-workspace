...
class LanguageCompleter : public QObject
{
    Q_OBJECT
public:
    explicit LanguageCompleter(QStringList packages, QObject *parent = nullptr)
        : QObject(parent)
...
protected:
    explicit CompletionCheck(QString languageCode, QObject *parent = nullptr)
        : QObject(parent)
        , m_languageCode(std::move(languageCode))
    {
    }
...
void Language::reloadCompleteness()
{
    if (state == State::Missing) {
        return; // this language is entirely missing. no point checking completeness
    }
